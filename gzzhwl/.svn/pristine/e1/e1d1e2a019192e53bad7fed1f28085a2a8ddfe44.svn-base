...
public class TripManageServiceImpl implements TripManageService {
	...
	public List<Map<String, Object>> getFieldControlList(String addrId, String keyWord, String timeStamp) {
		...
		if (!ValidateUtils.isEmpty(loadList)) {
			for (Map<String, Object> map : loadList) {
				...
				String orderGoodsWeight = (String) map.get("orderGoodsWeight");
				if(StringUtils.isNotBlank(orderGoodsWeight)) {
					map.put("orderGoodsWeight", getGoodWeight(orderGoodsWeight));
				}
							
				String consignGoodsWeight = (String) map.get("consignGoodsWeight");
				if(StringUtils.isNotBlank(consignGoodsWeight)) {
					map.put("consignGoodsWeight", getGoodWeight(consignGoodsWeight));
				}
	...
	public List<Map<String, Object>> getFieldControlHis(String staffId) {
		...
		params.put("statusArray",
				new String[] { LoadBillType.CANCEL_TRIP.toString(), LoadBillType.HAS_INVALID.toString(),
						LoadBillType.DEPART.toString(), LoadBillType.ARRIVED.toString(),
						LoadBillType.ELECRECEIPT.toString(), LoadBillType.PRINTRECEIPT.toString(),
						LoadBillType.CLOSED.toString() });
		...
		if (!ValidateUtils.isEmpty(loadHis)) {
			for (Map<String, Object> map : loadHis) {
				...
				String goodsWeight = (String) map.get("goodsWeight");
				map.put("goodsWeight", getGoodWeight(goodsWeight));
			}
		}
