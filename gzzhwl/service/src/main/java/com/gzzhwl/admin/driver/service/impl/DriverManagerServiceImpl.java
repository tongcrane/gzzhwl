...
public class DriverManagerServiceImpl implements DriverManagerService {
	...
	public String update(RealDriverInfo driverInfo, DriverBankInfo bankInfo, String staffId) {
		...
		RealDriverInfo oldInfo = getDriverInfo(driverInfoId);
		DriverManageValidate.valid_not_exist(ValidateUtils.isEmpty(oldInfo));
		...
		// 联动维护司机运势界账户信息
		accountService.updateAccountForCBS(driverInfo.getIdno(), driverInfo.getRealName(), driverInfo.getTelphone(),
				oldInfo.getTelphone(), driverInfo.getIdFImageRefId(), driverInfo.getIdBImageRefId(), staffId);
		return driverInfo.getDriverInfoId();
	}

	public RealDriverInfo getDriverInfo(String driverInfoId) {
	...
	public String save(RealDriverInfo driverInfo, DriverBankInfo bankInfo, String staffId, Integer departId) {
		...
		accountService.createAccountForCBS(driverInfo.getIdno(), driverInfo.getRealName(), driverInfo.getTelphone(),
				driverInfo.getIdFImageRefId(), driverInfo.getIdBImageRefId(), staffId);
		return driverInfoId;
	}
	...
	@Override
	public Page<Map<String, Object>> page(DriverInfoQueryVo queryVo, int current, int pagesize, String queryType) throws ParseException {
	...
	public Map<String, Object> getDriverDetail(String driverInfoId) {
		Map<String, Object> driverInfo = realDriverDao.findOne(driverInfoId);
		return driverInfo;
	}
