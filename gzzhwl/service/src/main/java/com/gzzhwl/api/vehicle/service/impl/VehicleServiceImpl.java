...
public class VehicleServiceImpl implements VehicleService {
	...
	public String updateVehicleInfo(VehicleInfo vehicleInfo, VehiclePlusInfo vehicleInfoPlusInfo, String accountId,
		...
		if (!res) {
			throw new RestException(ErrorCode.ERROR_900012);
		}
	...
	public boolean realVehicleExistValidator(String plateNumber) {
		...
		if (listMap.size() > 1) {
			throw new RestException(ErrorCode.ERROR_800027);
		}
	...
	public String updateRealVehicle(RealVehicleInfo vehicleInfo, RealVehiclePlusInfo vehicleInfoPlusInfo) {
		...
		vehicleDao_real.updateSelective(vehicleInfo);

		if (vehicleInfoPlusInfo != null) {
			if (ValidateUtils.isEmpty(vehicleInfoPlusInfo.getVehicleInfoId())) {
				throw new RestException(ErrorCode.ERROR_800001);
			}
			vehiclePlusInfoDao_real.updateSelective(vehicleInfoPlusInfo);
		}
	...
	@Override
	public VehicleInfoVo getVehicle(String vehicleInfoId) {
//		String status = "";
	...
	@Override
	public VehicleVo getVehicleInfo(String vehicleInfoId) {
		VehicleValidator.valid_vehicle_id(vehicleInfoId);
		...
		//先查正式表
		lists = realVehicleInfoExtDao.find(params);
		if(CollectionUtils.isEmpty(lists)) {
			//查临时表
			lists = vehicleInfoExtDao.find(params);
		}
		
		//验证车辆是否存在
		VehicleValidator.valid_exist(CollectionUtils.isNotEmpty(lists));
		
		Map<String, Object> vehicleMap = lists.get(0);
		VehicleVo vehicleVo = beanMapper.map(vehicleMap, VehicleVo.class);
