var CBSBidManageInfo = function(options){
    ...
    this.init = function (options) {
        ...
        if (options && options.sourceId) {
            ...
            var url_win="/admin/quotedmanage/getBidQuotedInfo";
            ...
            $.get(global.server + url_source, function (msg) {
                if (msg && msg.status && msg.status.statusCode == global.status.success) {
                    ...
                    var bidTime=data.bidTime;
                    if(bidTime) {
                    	bidTime=moment(bidTime, "YYYY-MM-DD HH:mm:ss").format("YYYY-MM-DD HH:mm");
                    }
                	$('.winbid_time>span').html(bidTime);
                    if(data.isShowVehicleInfo==false) {
                    	$('#vehicle_info').html("运输车辆信息：待完善");
                    	$('#show-vehicle-info').remove();
                    }
                    	...
                    	if(data) {
                    		...
                    		if(actionList) {
                    			$.each(actionList,function(i,j){
                    				if(j.code=='15') {
                    					$('.bid_close_action_15').show();
                    					$('.bid_close_action_15').click(function(){
                    						var options = {
                    	            			data : {quotedId : data.quotedId},
                    	            			text : '是否确认关闭',
                    	            			callback:function(data){
                    	            				_this.doInvalid(data.quotedId);
                    	            			}
                    	            		}
...
//                    						_this.doInvalid(data.quotedId);
                    		...
                    		if(data.status=='03'||data.status=='05'||data.status=='09'||data.status=='10') {
//                            	var bidTime=data.bidTime;
//                                if(bidTime) {
//                                	bidTime=moment(bidTime, "YYYY-MM-DD HH:mm:ss").format("YYYY-MM-DD HH:mm");
//                                }
...
//                                }
                    		...
                    		var el = $('<tr>\
                					<td id="win-telphone"></td>\
                					<td id="win-realName"></td>\
                					<td id="win-idno"></td>\
                					<td id="win-price"></td>\
                					<td id="win-remark"></td>\
                					<td id="win-status"></td>\
                					<td id="win-hisRemark"></td>\
                					<td id="win-completed"></td>\
                					<td id="win-invalid"></td>\
                					<td >\
                						<div class="owe-to">\
                				    		<span class="owe-span">\
                				    			<ul class="deadline">\
                        	...
                        	var price=global.defaultIfBlack(data.price,'');
                			price=price?price+' 元':'';
                			el.find('#win-telphone').html(global.defaultIfBlack(data.telphone,''));
                			el.find('#win-realName').html(global.defaultIfBlack(data.realName,''));
                			el.find('#win-idno').html(global.defaultIfBlack(data.idno,''));
                			el.find('#win-price').html(price);
                			el.find('#win-remark').html(global.defaultIfBlack(data.remark,''));
                			el.find('#win-status').html(global.defaultIfBlack(data.statusCn,''));
                			el.find('#win-hisRemark').html(global.defaultIfBlack(data.hisRemark,''));
                			el.find('#win-completed').html(global.defaultIfBlack(data.completed,''));
                			el.find('#win-invalid').html(global.defaultIfBlack(data.invalid,''));
                			el.find('#win-createdTime').html(global.defaultIfBlack(data.createdTime,''));
                			$('.win-bid>table>tbody').append(el);
                    	}
                    } else {
                    	alert("页面参数错误，或数据已不存在。");
                    	window.close();
                    ...
                    if (msg && msg.status && msg.status.statusCode == global.status.success) {
                        ...
                        if(driverList) {
                        	if(driverList.length==0) {
                                ...
                                $('#show-vehicle-info').removeClass('active').css('cursor','pointer');
                                $('#show-vehicle-info').show();
//                                var href = global.getContextPath() + '/NewDriverInfo.html?accountId='+data.accountId+'&vehicleInfoId='+data.vehicleInfoId;
                                $('#show-vehicle-info').data('accountId',data.accountId);
                                $('#show-vehicle-info').data('vehicleInfoId',data.vehicleInfoId);
                                $('#vehicle_info').html("运输车辆信息："+car_num+" 司机："+driverInfoList);
                                
                                $('#show-vehicle-info').click(function(){
            ...
            var table = new CBSTable({
        		page : $('.handle-page'),
        		content : $('.all-quoted > table > tbody'),
        		url : '/admin/quotedmanage/queryAllQuotedList',
        		method  : 'get',
        		pageNumber : 0,
        		pageSize : 10,
        		rowRender : function(index, row){
        			...
        			var sourceStatus=row.sourceStatus;
        			if(sourceStatus=='04'||sourceStatus=='05') {
        				el.find('.opt-name').remove();
