...
CBSCustom.prototype.fillContentFromJsonData = function (data) {
    ...
    if(data.type=='01') {
    	$('.toggle_label').html('身份证号码');
    	$('.people').attr('checked','checked');
    } else if(data.type=='02') {
    	$('.toggle_label').html('税务登记证号');
...
CBSCustom.prototype.validToPost = function () {
    ...
    $.each(this.pcdList,function(i, j){
		...
		if(_p==''||_c==''||_d==''){
			valid = false;
			$('div[data-column="'+ result.category+'"]').siblings('.tips').show();
		} else {
			if(address != ''){
				$('div[data-column="'+ result.category+'"]').siblings('.tips').hide();
			}
		}
	});
...
CBSCustom.prototype.validKeyExpress=function() {
	...
	if(type.val()=='01') {
		if(taxRegCode.val()!='') {
			...
			valid=false;
			taxRegCode.siblings('.tips').html('请输入身份证号');
			taxRegCode.siblings('.tips').show();
		}
	} else if(type.val()=='02') {
		if(taxRegCode.val()!='') {
			...
			valid=false;
			taxRegCode.siblings('.tips').html('请输入税务登记号');
			taxRegCode.siblings('.tips').show();
		}
	}
