	...
	this.el=$('<div class="mo_del"> \
		<div class="model_content mask"> \
			<h4> \
				<p>跟踪信息</p> \
				<div class="radio_btn"> \
					<label><input type="radio" name="status" value="01" checked="checked">无异常</label> \
					<label><input type="radio" name="status" value="02">有异常</label> \
				</div> \
			</h4> \
			<div class="mask_nor"> \
				<div class="layout"> \
...
var CBSTrackArrive=function(options){
	this.opt = $.extend({}, options);
	
	this.el=$('<div class="mo_del">\
					<div class="model_content arrive">\
						<h4>车辆到达</h4>\
						<div class="layout">\
							<label class="labe_l" for="">到达时间</label>\
							<input class="inpu_t use-date" type="text" data-time="true" data-column="realArriveTime">\
						</div>\
						<a href="javascript:void(0);" class="">提交</a>\
						<i class="clo_se"></i>\
					</div>\
...
//		_this.el.find('._title').html(_this.opt.title);
		var realArriveTime=moment(new Date()).format("YYYY-MM-DD HH:mm");
		_this.el.find('.use-date').val(realArriveTime);
		_this.el.find('.use-date').each(function(i, j){
    		var time = $(this).data('time');
    		if(time){
    			$(this).datetimepicker({
	...
	this.getData = function(loadId){
		...
		$.get(global.server+'/admin/transit/getTransitDetail',{loadId:loadId}, function(msg){
			if (msg && msg.status && msg.status.statusCode == global.status.success) {
				var data = msg.data;
				var loadNo=data.loadVehicleInfo.loadNo;
				_this.data = data;
			...
			$('.arrive_car').click(function(){
				var options = {
						data : {},
						callback:function(data){
							_this.ArriveTheCar(loadId);
						}
					}
					new CBSTrackArrive(options);
			});
...
//        		el.find('.ab_a').show();
    		...
    		el.find('.track_time').text(feedbackTime);
    		if(j.realName) {
    			if(j.position) {
...
//车辆到达
CBSTrackInfo.prototype.ArriveTheCar=function(loadId) {
	var _this=this;
	var url='/admin/arrived/arrive';
	var data = {};
	data.arriveTime=$('input[data-column="realArriveTime"]').val();
	data.loadId=loadId;
	var valid=this.validArrive(data);
	if(valid) {
		$.ajax({
...
CBSTrackInfo.prototype.validArrive=function(data) {
	var valid=true;
	if(data.arriveTime==''||data.arriveTime==null) {
		valid=false;
		alert('实际到达时间不能为空');
		return false;
	}
