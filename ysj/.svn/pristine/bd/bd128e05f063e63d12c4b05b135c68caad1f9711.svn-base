var CBSImage = function(options) {
	...
	this.dropPhoto = function(data) {
		...
		target.find('.picture').remove();
		_this.fillData(data);
		target.append(this.el);
		var isInit = data.init;
		if(!isInit){
			if(_this.opt.onSelect instanceof Function){
				_this.opt.onSelect();
			}
		}
	};
	...
	this.init = function(options) {
		...
		target.append($('<input type="file" accept="image/jpg,image/gif,image/png" class="hide"/>'));
...
//		
		target.on('click', function() {
			var trigger = target.find('input:file')[0];
