	...
	$.fn.GdMap = function(options){
	    ...
	    _this.showCarInfo = function(map){
			...
			for(var j=0;j< markers.length;j++){
				...
				if(j>1){
					if(options.speed[j-2] === null || options.speed[j-2] === undefined){
						curSped = "NA"
					}else{
						if(options.speed[j-2]!='NA'){
							curSped = options.speed[j-2]+" km/h";
						} else{
							curSped = options.speed[j-2];
						}
					}
					lng = markers[j].position[1];
					lat = markers[j].position[0];
					...
					if(markers[j].exception == "01"){
						...
						info.push('<span>位置：'+lng+","+ lat+'</span></div></div></div>');
					}else{
				        //构建信息窗体中显示的内容
				        ...
				        info.push('<span>位置：'+lng+','+ lat+'</span></div>');
				        info.push('</div>');
					}
				}else{
					//设置起点终点的信息
