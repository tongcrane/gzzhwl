...
var CBSAddLoad = function(options){
	...
	this.init = function(options){
		...
		if (options && options.loadId) {
            ...
            var _this = this;
            $.get(global.getServer() + url, function (msg) {
	...
	$('.isPredict_01').click(function(){
		$(this).addClass('checked');
		$('.isPredict_02').removeClass('checked');
		$('div[data-column="isPredict"]').val('01');
	})
	$('.isPredict_02').click(function(){
		$('.isPredict_01').removeClass('checked');
		$(this).addClass('checked');
		$('div[data-column="isPredict"]').val('02');
	});
...
CBSAddLoad.prototype.showLoadInfo = function(){
	...
	if(status != '07') {
		...
		if(loadInfo.isPredict){
			//02预估  01准确
			var isPredict=loadInfo.isPredict;
			if(isPredict=='01') {
				$('.isPredict_01').addClass('checked');
			} else if(isPredict=='02') {
				$('.isPredict_02').addClass('checked');
			}
			$('div[data-column="isPredict"]').val(isPredict);
//			$('input:radio[name=isPredict][value="'+loadInfo.isPredict+'"]').prop('checked',true);
		}
		
		if(loadInfo.bridgeType){
			//01单桥  02双桥
			var bridgeType=loadInfo.bridgeType;
			if(bridgeType=='01') {
				$('.bridgeType_01').addClass('checked');
			} else if(bridgeType=='02') {
				$('.bridgeType_02').addClass('checked');
			}
			$('div[data-column="bridgeType"]').val(bridgeType);
...
CBSAddLoad.prototype.createLoad = function(){
	...
	if(valid){
	    ...
	    $.ajax({
	        url: global.getServer() + '/admin/load/create',
	        type: "POST",
...
CBSAddLoad.prototype.modifyLoad = function(){
	...
	if(valid){
		var dataStringify = JSON.stringify(data);
	    $.ajax({
	        url: global.getServer() + '/admin/load/modify',
	        type: "POST",
	        contentType: "application/json; charset=utf-8",
	        data: dataStringify,
	        dataType: "json",
...
CBSAddLoad.prototype.cancelLoad = function(){
    ...
    $.ajax({
        url: global.getServer() + '/admin/load/cancel',
        type: "POST",
        data: {loadId:loadId},
        success: function (msg) {
            if (msg && msg.status && msg.status.statusCode == global.status.success) {
