...
//	}
	...
	this.init = function() {
		...
		_this.page.on('click','.pagination-spec',function(e){
			var toPage = parseInt($(this).data('page'));
			_this._switchToPage(toPage, true);
		});
		
		_this.page.on('click','.pagination-prev',function(e){
			_this._switchToPage((_this.currentPage-1), true);
		});

		_this.page.on('click','.pagination-next',function(e){
			_this._switchToPage((_this.currentPage+1), true);
		});

		_this.page.on('click','.pagination-jump',function(e){
			_this._switchToPage(parseInt(_this.page.find(".pagination-goto").val()), true);
		})

		_this.page.on('keyup','.pagination-goto',function(e){
	...
	this._resetPagination = function() {
		...
		paginationInner += '<input class="pagination-goto" type="tel" value="' + (((currPage+1)<totalPage)?(currPage+1):totalPage) + '">';
		paginationInner +='页';
		paginationInner += '<button class="pagination-jump">确定</button>'
