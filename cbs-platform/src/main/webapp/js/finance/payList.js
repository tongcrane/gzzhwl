...
//	new CBSPCSelect($("#endCodeP"),$('#endCodeC'),'.search-content');
	...
	var table = new CBSTable({
		...
		rowRender : function(index, row){
			...
			var oilPay=row.oilPay;
			if(oilPay) {
				if(!isNaN(oilPay)) {
					oilPay=Number(oilPay).toFixed(2);
				}
			}else {
				oilPay = "0.00";
			}
			...
			el.find('.prePay').html(prePay);
			el.find('.oilPay').html(oilPay);
			el.find('.otherPrice').html(otherPrice);
