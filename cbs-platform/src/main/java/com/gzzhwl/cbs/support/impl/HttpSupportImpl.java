...
public class HttpSupportImpl implements HttpSupport {
	...
	public <K, V> String postAsForm(final String url, final Map<K, V> param, HttpConfig config) throws HttpException {
		...
		return doPost(url, body, ContentType.APPLICATION_FORM_URLENCODED.withCharset(UTF_8), config);
	}

	@Override
	public String postAsBody(final String url, final String body, HttpConfig config) throws HttpException {
		return doPost(url, body, ContentType.APPLICATION_JSON.withCharset(UTF_8), config);
	}
	...
	private String doPost(final String url, final String body, final ContentType contentType, HttpConfig config)
