...
var global = {
	"getContextPath" : function(){
    	return global_ctxPath;
    },
    "getServer" : function(){
    	return global_server;
    },
    "getAuthorization" : function(){
		if("undefined" != typeof global_authorization){
			return global_authorization;
		} else{
    		return "";
    	}
    },
    ...
    "imgsrc": function (id, opt) {
    	var url = global.getServer() +'/api/image/'+id + "?";
    	for(var p in opt){
    		var param = p + "=" + opt[p] +"&";
    		url += param;
    	}
...
(function($) {
    ...
    $.ajax = function(settings) {
    	...
    	if(global.getAuthorization() != ''){
    		settings.headers =  {
		        'Authorization': global.getAuthorization()
		    }
    	}
    	settings.error = function(xhr, status, e) {
    		if (xhr.status == 403) {
    			location.href = global.getContextPath()+'/logout';
    		} else {
...
//            return global.ajaxstate;
        ...
        settings.complete = function (obj,status) {
            if(status == 'error'){
