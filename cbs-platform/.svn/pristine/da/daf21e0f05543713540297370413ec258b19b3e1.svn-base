...
var CBSAddOrder = function(options){
	...
	this.selectAgreement = function(agreementId){
		if(agreementId != ''){
			$.get(global.getServer() + '/admin/agreement/getAgreementDetail', {agreementId:agreementId}, function(msg){
...
//	 		$(this).append('<select class="pcd-select-d"></select>');
		...
		$('.baseInfo-column').each(function(i, j){
			...
			} else if(_p=='isPredict'){
				var isPredict=baseInfo[_p];
				if(isPredict=='01') {
	...
	this.getChargeData = function(){
		var _this = this;
		$.get(global.getServer() + '/admin/order/charge', function (msg) {
            if (msg && msg.status && msg.status.statusCode == global.status.success) {
                var data = msg.data;
...
//	}
	...
	this.getAgreementData = function(custId, initId){
		_this=this;
		if(custId != null){
			$.get(global.getServer() + '/admin/agreement/getAgreements', {custId:custId}, function(msg){
				if (msg && msg.status && msg.status.statusCode == global.status.success) {
					var data = msg.data;
